...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<object> result = from e in Emps
                                         group e by e.Job into LiczbaPracowników
                                         select new
                                         {
                                             Praca = LiczbaPracowników.Key,
                                             LiczbaPracownikow = LiczbaPracowników.Count()
                                         };
            return result;
        }
        ...
        {
            Emp result = Emps.OrderByDescending(e=> e.HireDate).FirstOrDefault(e => e.Job== "Frontend programmer");
            return result;
        }
        ...
        {
            IEnumerable<Emp> list2 = new List<Emp>() { new Emp {Ename="brak wartości", Job=null, HireDate=null } };
            IEnumerable<object> result = Emps.Select(e => new { e.Ename, e.Job, e.HireDate }).Union(list2.Select(e=> new
            {
        ...
        public static IEnumerable<object> Task11()
        {
            IEnumerable<object> result = from e in Emps
                                         ...
                                         group e by d.Dname into numOfEmployess
                                         where numOfEmployess.Count()>1
                                         select new
                                         {
                                             name = numOfEmployess.Key,
                                             numOfEmployess = numOfEmployess.Count()
                                         };
            return result;
        }
        ...
        {
            IEnumerable<Emp> result = Emps.GetEmpsWithSubordinates().OrderBy(e=>e.Ename).OrderByDescending(e=>e.Salary);
        ...
        public static IEnumerable<object> Task14()
        {
            IEnumerable<object> result = from d in Depts
                                       ...
                                       group e by d.Dname into numOfEmployess
                                       where numOfEmployess.Count() > 5 || numOfEmployess.Any()
                                       orderby numOfEmployess.Key descending
                                       select numOfEmployess.Key;
            //result =
            return result;
        }
    }
    ...
    public static class CustomExtensionMethods
    {
        ...
        {
            List<Emp> empl = new List<Emp>();
            foreach(Emp e in emps)
            {
                if(e.Mgr != null)
                {
                    if(!empl.Contains(e.Mgr))
                    {
                        empl.Add(e.Mgr);
                    }
                }
